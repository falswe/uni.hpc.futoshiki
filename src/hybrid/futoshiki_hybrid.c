...
// These are the actual definitions for the variables declared as 'extern' in the header.
int g_mpi_rank = 0;
int g_mpi_size = 1;
...
static bool solve_work_unit_omp(Futoshiki* puzzle, WorkUnit* received_wu, int solution[MAX_N][MAX_N]);
...
// CHANGED: Function name now matches the header and call from main.c
...
// CHANGED: Function name now matches the header
void finalize_hybrid(void) {
    MPI_Finalize();
}

// NEW: Setter function for MPI task factor, called from main.c
...
// NEW: Setter function for OpenMP task factor, called from main.c
...
static void hybrid_worker(Futoshiki* puzzle) {
    ...
    while (true) {
        ...
        if (solve_work_unit_omp(puzzle, &work_unit, local_solution)) {
            ...
            MPI_Recv(&work_unit, sizeof(WorkUnit), MPI_BYTE, 0, TAG_TERMINATE, MPI_COMM_WORLD, &status);
            break;
        }
    }
}
...
static bool hybrid_master(Futoshiki* puzzle, int solution[MAX_N][MAX_N]) {
    ...
    while (active_workers > 0) {
        ...
        if (status.MPI_TAG == TAG_SOLUTION_FOUND) {
            if (!found_solution) {
                found_solution = true;
                MPI_Recv(solution, MAX_N * MAX_N, MPI_INT, worker_rank, TAG_SOLUTION_DATA, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                log_verbose("Master received solution from worker %d. Shutting down.", worker_rank);
                MPI_Send(&dummy_unit, sizeof(WorkUnit), MPI_BYTE, worker_rank, TAG_TERMINATE, MPI_COMM_WORLD);
                active_workers--;
            } else {
                int temp_sol[MAX_N * MAX_N];
                MPI_Recv(&temp_sol, MAX_N * MAX_N, MPI_INT, worker_rank, TAG_SOLUTION_DATA, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                MPI_Send(&dummy_unit, sizeof(WorkUnit), MPI_BYTE, worker_rank, TAG_TERMINATE, MPI_COMM_WORLD);
                active_workers--;
            }
            ...
            if (found_solution || next_unit >= num_units) {
                ...
                log_verbose("Terminating worker %d (%s). %d workers left.", worker_rank,
                            found_solution ? "solution found by other" : "no more work", active_workers);
            } else {
                MPI_Send(&work_units[next_unit], sizeof(WorkUnit), MPI_BYTE, worker_rank, TAG_WORK_ASSIGNMENT, MPI_COMM_WORLD);
                next_unit++;
            }
        }
    }

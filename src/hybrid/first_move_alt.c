...
//                             if (color_g_seq(puzzle, thread_local_solution, start_row,
//                                             start_col + 1)) {
// #pragma omp critical
//                                 {
//                                     if (!found_solution_local) {
...
// static bool color_g_hybrid(Futoshiki* puzzle, int solution[MAX_N][MAX_N]) {
//     if (g_mpi_size <= 1) {
//         log_info("Only 1 MPI process. Falling back to OpenMP-only solver on a single node.");
...
//         // A better way is to have master return a bool. Let's assume master updates a shared
//         // status. For now, we rely on the fact that `solution` is filled. A simple check: if the
//         // first cell is non-zero (assuming it was initially).
